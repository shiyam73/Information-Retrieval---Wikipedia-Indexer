...
public class SingleIndexerRunner {
	 ...
	 */
	protected void processTokenMap(int docid, Map<String, Integer> map)
			throws IndexerException {
		String key;
		int value;
		Object[] arrObj;
		...
		if (docCount % 2000 == 0 && docCount > 1) {
			...
			System.out.println("Single Indexed Runner writing to disk::"+ docCount);
		}
	}

	protected boolean isFinished() {
		return thr.isComplete && thr.isQueueEmpty() && idxWriter.isComplete;
	}
	...
	private class RunnerThread implements Runnable {
		...
		private void setComplete() {
			System.out.println("Set Complete for SIR!!");
			isComplete = true;
		}
		...
		public void run() {
			...
			while (true) {
				...
				if (etr == null) {
					if (isComplete) {
						try {
							System.out.println("Writing to file for one last time Single Indexed runner!!!");
							idxWriter.writeDict();
							idxWriter.writeToDisk();
							System.out.println("Write to disk completed for SIR!!");							
					...
					try {
						if (lookupBoth) {
							idxWriter.addToIndex((Integer) etr[0],
									(Integer) etr[1], (Integer) etr[2]);
						} else {
							idxWriter.addToIndex((String) etr[0],
									(Integer) etr[1], (Integer) etr[2]);
						}
					} catch (IndexerException e) {
						// TODO Auto-generated catch block
